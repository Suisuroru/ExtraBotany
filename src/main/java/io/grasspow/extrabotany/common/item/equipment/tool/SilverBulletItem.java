...
public class SilverBulletItem extends ManaBlasterItem {
    ...
    public @NotNull InteractionResultHolder<ItemStack> use(Level world, Player player, @NotNull InteractionHand hand) {
        ...
        if (player.isSecondaryUseActive() && hasClip(stack)) {
            ...
            if (burst != null && (((ShootingGuardianArmorItem) ExtraBotanyItems.SHOOTING_GUARDIAN_HELM.get()).hasArmorSet(player) || ManaItemHandler.instance().requestManaExact(stack, player, burst.getMana(), true))) {
                if (!world.isClientSide) {
